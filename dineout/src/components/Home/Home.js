    ...
    searchRestaurants = async (event) => {
        ...
        const searchtext = this.state.search.toLowerCase();
            ...
            const searchData = restaurantsData.data.filter(restaurantObj => {
               return restaurantObj.restName.toLower().startsWith(searchtext) || restaurantObj.restCity.toLower().startsWith(searchtext);
            });
            this.setState({
                ...
                <div className = "col-sm-12">
                    <div className = "row">
                        { this.state.restaurants && this.state.restaurants.length > 0 ? this.state.restaurants.map(restaurant => 
                           <div className = "col-sm-12 col-md-6 col-xl-4" key={restaurant.id} >
                                <Link to = {{
